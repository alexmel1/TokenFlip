...
export default function Home() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('flip_history_v3');
  ...
  const handleJoinSuccess = async () => {
    ...
    // Анимация и проверка результата через 7 секунд
    setTimeout(async () => {
      const { data: newB } = await refetchUSDC();
      const currentBalance = newB?.value || BigInt(0);
      
      // Логика победы: если баланс вырос
      const isWin = currentBalance > oldBalance;
      setWinStatus(isWin ? 'win' : 'lose');
      setGameState('result');

      const newEntry = { status: isWin ? 'WIN' : 'LOSE', amount, time: Date.now() };
      const newHistory = [newEntry, ...history].slice(0, 10);
      setHistory(newHistory);
      localStorage.setItem('flip_history_v3', JSON.stringify(newHistory));
      refreshLobby();
    }, 7000);
  };
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#020617', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .overlay { position: fixed; inset: 0; background: rgba(2,6,23,0.98); z-index: 1000; display: flex; flex-direction: column; align-items: center; justify-content: center; backdrop-filter: blur(20px); }
